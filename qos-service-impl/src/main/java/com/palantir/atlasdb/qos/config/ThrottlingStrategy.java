...
public interface ThrottlingStrategy {
    ...
    enum ThrottlingStrategies {
        SIMPLE(new SimpleThrottlingStrategy());

        private final ThrottlingStrategy throttlingStrategy;

        ThrottlingStrategies(ThrottlingStrategy throttlingStrategy) {
    ...
    double clientLimitMultiplier(List<CassandraHealthMetricMeasurement> metricMeasurements,
