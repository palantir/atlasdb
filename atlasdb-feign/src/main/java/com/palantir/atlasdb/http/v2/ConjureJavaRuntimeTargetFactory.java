...
public final class ConjureJavaRuntimeTargetFactory implements TargetFactory {
    ...
    @Override
    public <T> T createProxy(Optional<TrustContext> trustContext,
            String uri,
            Class<T> type,
            String userAgent,
            boolean limitPayloadSize) {
        ClientConfiguration clientConfiguration = ClientOptions.FAST_RETRYING_FOR_TEST
                .create(ImmutableList.of(uri), Optional.empty(), trustContext.orElseThrow(
                        () -> new IllegalStateException("CJR requires a trust context")));
        return JaxRsClient.create(type, createAgent(userAgent), new HostMetricsRegistry(), clientConfiguration);
    }

    @Override
    public <T> T createProxyWithFailover(Optional<TrustContext> trustContext,
        ...
        return wrapInRetryingProxy(JaxRsClient.create(
                type,
                UserAgents.tryParse(userAgent).addAgent(AtlasDbRemotingConstants.ATLASDB_HTTP_CLIENT_AGENT),
    ...
    private static UserAgent createAgent(String userProvidedAgent) {
        return UserAgents.tryParse(userProvidedAgent).addAgent(AtlasDbRemotingConstants.ATLASDB_HTTP_CLIENT_AGENT);
    }
}
