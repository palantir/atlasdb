...
public class AnnotatedCallableTest {
    ...
    @Test
    public void changingThreadNameIsNoOp() {
        Stream.of(AnnotationType.values()).forEach(type -> {
            Callable<String> callable = AnnotatedCallable.wrapWithThreadName(AnnotationType.PREPEND, "test", () -> {
                assertThat(Thread.currentThread().getName()).isEqualTo("AnnotatedCallableTest");
                return "Hello, world!";
            });
