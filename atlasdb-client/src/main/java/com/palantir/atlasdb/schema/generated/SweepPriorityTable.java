...
public final class SweepPriorityTable
  ...
  private static final String rawTableName = "priority";
  private final TableReference tableRef;
  private static final ColumnSelection allColumns =
      getColumnSelection(SweepPriorityNamedColumn.values());
  ...
  static SweepPriorityTable of(
      Transaction t,
      Namespace namespace,
      SweepPriorityTrigger trigger,
      SweepPriorityTrigger... triggers) {
    return new SweepPriorityTable(
        t,
        namespace,
        ImmutableList.<SweepPriorityTrigger>builder().add(trigger).add(triggers).build());
  }

  static SweepPriorityTable of(
      Transaction t, Namespace namespace, List<SweepPriorityTrigger> triggers) {
    return new SweepPriorityTable(t, namespace, triggers);
  }

  private SweepPriorityTable(
      Transaction t, Namespace namespace, List<SweepPriorityTrigger> triggers) {
    this.t = t;
  ...
  public static final class SweepPriorityRow implements Persistable, Comparable<SweepPriorityRow> {
    ...
    public static final Hydrator<SweepPriorityRow> BYTES_HYDRATOR =
        new Hydrator<SweepPriorityRow>() {
          @Override
    ...
    public int compareTo(SweepPriorityRow o) {
      return ComparisonChain.start().compare(this.fullTableName, o.fullTableName).result();
    }
  }
  ...
  public static final class LastSweepTime implements SweepPriorityNamedColumnValue<Long> {
    ...
    public static final Hydrator<LastSweepTime> BYTES_HYDRATOR =
        new Hydrator<LastSweepTime>() {
          @Override
  ...
  public static final class WriteCount implements SweepPriorityNamedColumnValue<Long> {
    ...
    public static final Hydrator<WriteCount> BYTES_HYDRATOR =
        new Hydrator<WriteCount>() {
          @Override
  ...
  public interface SweepPriorityTrigger {
    public void putSweepPriority(
        Multimap<SweepPriorityRow, ? extends SweepPriorityNamedColumnValue<?>> newRows);
  }
  ...
  public static ColumnSelection getColumnSelection(Collection<SweepPriorityNamedColumn> cols) {
    return ColumnSelection.create(
        Collections2.transform(cols, SweepPriorityNamedColumn.toShortName()));
  }
  ...
  private static final Map<String, Hydrator<? extends SweepPriorityNamedColumnValue<?>>>
      shortNameToHydrator =
  ...
  public void putWriteCount(Map<SweepPriorityRow, Long> map) {
    Map<SweepPriorityRow, SweepPriorityNamedColumnValue<?>> toPut =
  ...
  public void putUnlessExists(
      Multimap<SweepPriorityRow, ? extends SweepPriorityNamedColumnValue<?>> rows) {
    Multimap<SweepPriorityRow, SweepPriorityNamedColumnValue<?>> existing =
        getRowsMultimap(rows.keySet());
    Multimap<SweepPriorityRow, SweepPriorityNamedColumnValue<?>> toPut = HashMultimap.create();
    for (Entry<SweepPriorityRow, ? extends SweepPriorityNamedColumnValue<?>> entry :
        rows.entries()) {
  ...
  @Override
  public List<SweepPriorityRowResult> getRows(
      Iterable<SweepPriorityRow> rows, ColumnSelection columns) {
  ...
  @Override
  public List<SweepPriorityRowResult> getAsyncRows(
      Iterable<SweepPriorityRow> rows, ExecutorService exec) {
    return getAsyncRows(rows, allColumns, exec);
  }

  @Override
  public List<SweepPriorityRowResult> getAsyncRows(
      final Iterable<SweepPriorityRow> rows, final ColumnSelection columns, ExecutorService exec) {
  ...
  @Override
  public List<SweepPriorityNamedColumnValue<?>> getRowColumns(
      SweepPriorityRow row, ColumnSelection columns) {
    byte[] bytes = row.persistToBytes();
    RowResult<byte[]> rowResult = t.getRows(tableRef, ImmutableSet.of(bytes), columns).get(bytes);
    if (rowResult == null) {
      return ImmutableList.of();
    } else {
      List<SweepPriorityNamedColumnValue<?>> ret =
  ...
  @Override
  public Multimap<SweepPriorityRow, SweepPriorityNamedColumnValue<?>> getRowsMultimap(
      Iterable<SweepPriorityRow> rows) {
  ...
  @Override
  public Multimap<SweepPriorityRow, SweepPriorityNamedColumnValue<?>> getAsyncRowsMultimap(
      Iterable<SweepPriorityRow> rows, ExecutorService exec) {
  ...
  private Multimap<SweepPriorityRow, SweepPriorityNamedColumnValue<?>> getRowsMultimapInternal(
      Iterable<SweepPriorityRow> rows, ColumnSelection columns) {
    SortedMap<byte[], RowResult<byte[]>> results =
        t.getRows(tableRef, Persistables.persistAll(rows), columns);
  ...
  private static Multimap<SweepPriorityRow, SweepPriorityNamedColumnValue<?>>
  ...
  @Override
  public Map<SweepPriorityRow, BatchingVisitable<SweepPriorityNamedColumnValue<?>>>
      getRowsColumnRange(
          Iterable<SweepPriorityRow> rows, BatchColumnRangeSelection columnRangeSelection) {
    ...
    Map<SweepPriorityRow, BatchingVisitable<SweepPriorityNamedColumnValue<?>>> transformed =
    ...
    for (Entry<byte[], BatchingVisitable<Map.Entry<Cell, byte[]>>> e : results.entrySet()) {
      ...
      BatchingVisitable<SweepPriorityNamedColumnValue<?>> bv =
  ...
  @Override
  public Iterator<Map.Entry<SweepPriorityRow, SweepPriorityNamedColumnValue<?>>> getRowsColumnRange(
      Iterable<SweepPriorityRow> rows, ColumnRangeSelection columnRangeSelection, int batchHint) {
    ...
    return Iterators.transform(
        results,
        e -> {
          SweepPriorityRow row =
              SweepPriorityRow.BYTES_HYDRATOR.hydrateFromBytes(e.getKey().getRowName());
          SweepPriorityNamedColumnValue<?> colValue =
              shortNameToHydrator
  ...
  static String __CLASS_HASH = "/cakj3OTVXrcOrXfKrmajw==";
}
