...
public class EnableClientInterfaces implements MigrationAction {
    private final CassandraStateManager dc2StateManager;

    public EnableClientInterfaces(CassandraStateManager dc2StateManager) {
        this.dc2StateManager = dc2StateManager;
    }

    @Override
    public void runForwardStep() {
        dc2StateManager.enableClientInterfaces();
    }

    @Override
    public boolean isApplied() {
        InterfaceStates interfaceStates = dc2StateManager.getInterfaceState();
        return interfaceStates.gossipIsRunning()
                && interfaceStates.nativeTransportIsRunning()
                && interfaceStates.rpcServerIsRunning();
    }
}
