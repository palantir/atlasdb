...
CONTAINER_3=(':lock-impl:check' ':atlasdb-dbkvs-tests:check' ':atlasdb-tests-shared:check' ':atlasdb-ete-test-utils:check' ':atlasdb-cassandra:check' ':atlasdb-api:check' 'atlasdb-ete-tests:test' 'atlasdb-cassandra-multinode-tests:test' )
...
case $CIRCLE_NODE_INDEX in
    ...
    3) ./gradlew --profile --continue -x compileJava -x compileTestJava -x findbugsMain -x findbugsTest -x checkstyleMain -x checkstyleTest -PcassVer=3.7 ${CONTAINER_3[@]} ;;
    ...
    6) ./gradlew --profile --continue -x compileJava -x compileTestJava -PcassVer=3.7 atlasdb-cassandra-integration-tests:test findbugsMain findbugsTest checkstyleMain checkstyleTest && checkDocsBuild ;;
esac
