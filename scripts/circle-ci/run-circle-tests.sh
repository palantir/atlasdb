...
CONTAINER_3=(':atlasdb-dbkvs:check' ':atlasdb-cassandra:check' ':timelock-server:integTest' ':lock-impl:check' ':atlasdb-dbkvs-tests:postgresTest' ':atlasdb-ete-test-utils:check' ':atlasdb-ete-tests:longTest')
...
CONTAINER_5=(':timelock-server:suiteTest')

CONTAINER_6=(':timelock-server:stressTest')

CONTAINER_7=(':atlasdb-ete-tests:timeLockMigrationTest')
...
case $CIRCLE_NODE_INDEX in
    ...
    11) ./gradlew $BASE_GRADLE_ARGS ${CONTAINER_10[@]} --stacktrace -PenableErrorProne=true && checkDocsBuild ;;
esac
