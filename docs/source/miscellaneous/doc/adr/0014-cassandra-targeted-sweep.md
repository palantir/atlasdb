...
2. Manual Sweep, which can be triggered through a REST endpoint to perform an iteration or a full sweep for a given table
and start row.

Note that there are corner cases where the sweep timestamp may be mistaken: a read only transaction that has been running
for longer than one hour (which is also subject to clock drift), or when a write transaction loses its lock while committing. To
ensure correctness for read only transactions with respect to these cases, on tables that allow read only transactions we
defensively write a **garbage deletion sentinel**, which is an empty value at a timestamp of -1, i.e., before any valid start
timestamp. If a read only transactions encounters a sentinel, that signalizes there could have been a historic version it should be
able to read, but it has been deleted and the transaction must therefore abort.
...
obviously error-prone and subject to random failures, sicne if the manual sweep is interrupted for any reason, it will not be
...
conjunction with **ranged tombstones**, which delete all versions of a cell between two timestamps, allows us to sweep all tables
...
The sweep queue is a persisted queue containing the relevant metadata for each write that was about to be committed into AtlasDB.
...
is *ts* divided by *50_000*. A fine timestamp partition of a write is the fine
...
case it is a delete. There are two currently supported strategies: `CONSERVATIVE` and `THOROUGH`. We also use a sharding strategy
...
#### sweepableCells
  ...
  data is persisted to the KVS, all writes from the transaction are partitioned into shards and also according to the sweep strategy
  ...
  **Value**: a `WriteReference` containing the remaining required metadata for targeted sweep: the `TableReference`, `Cell`, and
  ...
  Since a persisted size of a `Cell` object can be up to 3_000 bytes (dominating the size of the above entry), and we can have at
  most 12_500 transactions per row, by allowing at most 50 writes
  from the same transaction in a row, we ensure that the size of one non-dedicated row will not exceed 2 GB and should in practice
  be lower than 100 MB. We allow a maximum of 100_000 entries per dedicated row, which ensures that each dedicated row should not be
  ...
  transaction simply by multiplying the *timestamp_partition* by *50_000* and then adding its *timestamp_modulus*.
...
#### sweepableTimestamps
  ...
  **Value**: unused empty byte array.
  ...
  Locating the next row of *sweepableCells* to read is then done by reading the next column in the coarse partition we are currently
  in, or range scanning the much denser *sweepableTimestamps* table to locate the next coarse partition with entries and then reading
  the fine partition to look for.
...
#### sweepProgressPerShard
  ...
  *shard = -1* and *sweep_conservative = true*. Note that all writes to this table use atomic check and set, and the values are only
  allowed to be increased. A request to update a cell to a lower value will have no effect.

#### Writing to the Sweep Queue

Whenever a transaction is about to commit, before its writes are persisted into the KVS, `SerializableTransactionManager` enqueues
them to the sweep queue. Sweep queue creates a list of `WriteInfo`s containing the relevant information, partitions this list
...
number of shards that the sweep queue is using (the shard is determined from the hash of the `TableReference` and `Cell`). For each
of these partitions, we then put an entry into the *sweepableTimestamps* table for the start timestamp of the transaction. Finally,
for each of the partitions, we put the `WriteInfo`s into the *sweepableCells* table:
...
Reading from the sweep queue is done in the reverse order from writing. For a given shard, strategy, **minimum exclusive
timestmap** (targeted sweep reads this from the *sweepProgressPerShard* table as described later), and **maximum exclusive
...
Targeted Sweep has a number of background threads (as controlled the install config) continuously cycling through the shards for
each strategy. To find the next shard to sweep, the thread requests a TimeLock lock for the shard and strategy. If successful, it
...
thread successfully acquired a lock, we do the following
  ...
  4. Delete all writes that are referenced to from aborted transactions.
  5. Partition the remaining `WriteInfo`s by `Cell` and `TableReference`, and then take the greatest start timestamp from each
    ...
    are going to have a lower start timestamp and are therefore going to be deleted as well.
       - If the strategy is conservative: write a garbage collection sentinel, then put a ranged tombstone deleting all versions of that
...
### Benefits
- Targeted sweep can perform sweeps several orders of magnitude times faster than legacy sweep.
- The load on the underlying KVS is significantly reduced, particularly in the case of Cassandra.
