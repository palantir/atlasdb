...
.. csv-table::
   ...
   ``sweepThreads``, "Only specified in config", "1", "The number of threads to run sweep with. Changes require a restart of any service node to take effect. Not recommended for Cassandra KVS."
...
.. warning::
   ...
   However, while configured (and if the number of priority tables is at least the number of ``sweepThreads``), no table that is not marked as a priority table will ever be swept, meaning that old versions of cells will not be cleaned up.
