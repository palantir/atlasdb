/**
 * Copyright 2015 Palantir Technologies
 *
 * Licensed under the BSD-3 License (the "License");
 * you may not use this file except in compliance with the License.
 * You may obtain a copy of the License at
 *
 * http://opensource.org/licenses/BSD-3-Clause
 *
 * Unless required by applicable law or agreed to in writing, software
 * distributed under the License is distributed on an "AS IS" BASIS,
 * WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
 * See the License for the specific language governing permissions and
 * limitations under the License.
 */
package com.palantir.atlasdb.watcher;

import java.util.List;

import com.google.common.collect.Lists;
import com.palantir.atlasdb.watcher.ProgressWatcher.Reporter;

/**
 * Combines multiple Reporters into one
 *
 * @author jweel
 */
final class JointReporter implements Reporter {
    private final List<Reporter> reporters = Lists.newArrayList();

    public void add(Reporter reporter) {
        reporters.add(reporter);
    }

    @Override
    public void start(String name) {
        for (Reporter reporter : reporters) {
            reporter.start(name);
        }
    }

    @Override
    public void stop(String name, long durationMillis) {
        for (Reporter reporter : reporters) {
            reporter.stop(name, durationMillis);
        }
    }

    @Override
    public void step(String name, int steps, int maxSteps, String itemsName) {
        for (Reporter reporter : reporters) {
            reporter.step(name, steps, maxSteps, itemsName);
        }
    }

    @Override
    public void total(String pathName, long durationMillis) {
        for (Reporter reporter : reporters) {
            reporter.total(pathName, durationMillis);
        }
    }

    @Override
    public void say(String name, String format, Object... args) {
        for (Reporter reporter : reporters) {
            reporter.say(name, format, args);
        }
    }
}
