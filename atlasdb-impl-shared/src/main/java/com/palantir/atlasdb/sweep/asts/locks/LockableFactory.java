...
public final class LockableFactory<T> {
    ...
    private final ConcurrentMap<T, Lockable<T>> memoizedLocks =
            new MapMaker().weakValues().makeMap();
    private final TimelockService timelockService;
    private final Refreshable<Duration> lockTimeout;
    private final Function<T, LockDescriptor> lockDescriptorFunction;
    ...
    private LockableFactory(
        ...
        this.lockDescriptorFunction = lockDescriptorFunction;
    }
    ...
    public Lockable<T> createLockable(T lockable) {
        return memoizedLocks.computeIfAbsent(
                lockable,
                value -> Lockable.create(value, lockDescriptorFunction.apply(value), timelockService, lockTimeout));
    }
}
