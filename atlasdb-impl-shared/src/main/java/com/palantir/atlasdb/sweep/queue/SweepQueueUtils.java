...
public final class SweepQueueUtils {
    ...
    public static WriteInfo toWriteInfo(TableReference tableRef, Map.Entry<Cell, byte[]> write, long timestamp) {
        ...
        // Todo(snanda): we do not have a commitTs at this point. This is being used to enqueue sweep queue.
        ...
        writes.forEach(writesForShard -> {
            ...
            if (cellsWritten > MAX_CELLS_PER_SHARD) {
                throw new SafeIllegalArgumentException(
                        "Attempted to persist targeted sweep information for too many cells in the same shard,"
                                + " exceeding the maximum number of cells per shard per transaction."
