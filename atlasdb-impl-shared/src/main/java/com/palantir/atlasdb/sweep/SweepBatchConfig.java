...
public interface SweepBatchConfig {
    ...
    @Value.Derived
    static SweepBatchConfig min(SweepBatchConfig first, SweepBatchConfig second) {
        return ImmutableSweepBatchConfig.builder()
                .candidateBatchSize(Math.min(first.candidateBatchSize(), second.candidateBatchSize()))
                .deleteBatchSize(Math.min(first.deleteBatchSize(), second.deleteBatchSize()))
                .maxCellTsPairsToExamine(Math.min(first.maxCellTsPairsToExamine(), second.maxCellTsPairsToExamine()))
                .build();
    }
}
