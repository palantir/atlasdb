...
public final class DefaultCandidateSweepableBucketRetriever implements CandidateSweepableBucketRetriever {
    ...
    private DefaultCandidateSweepableBucketRetriever(
            SweepableBucketRetriever sweepableBucketRetriever, Refreshable<Duration> minimumDurationBetweenRefresh) {
    ...
    private Optional<TimestampedSweepableBuckets> getNext() {
        if (isWithinDebounceWindow()) {
            log.info("Skipping sweepable bucket update as we are within the debounce window. This is interesting"
                    + " because we passed the requestUpdate check.");
    ...
    private boolean isWithinDebounceWindow() {
        ...
        return existing != null
                && existing.timestamp()
                        .isAfter(Instant.now()
                                .minus(minimumDurationBetweenRefresh.get().plus(jitter)));
    }

    private void updateJitter() {
        jitter = Duration.ofMillis(random.nextInt(100));
        log.trace("Next refresh will have new jitter", SafeArg.of("jitter", jitter.toMillis()));
    }
    ...
    private final class SetRefreshable implements FutureCallback<Optional<TimestampedSweepableBuckets>> {
        ...
        public void onFailure(@NotNull Throwable throwable) {
            log.warn(
                    "There was an error retrieving the latest sweep buckets. The next request to update the sweepable"
                            + " buckets will retry the operation, but this likely indicates a bug.",
                    throwable);
        }
    }
}
