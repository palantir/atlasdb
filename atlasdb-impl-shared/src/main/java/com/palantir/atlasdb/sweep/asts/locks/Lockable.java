...
public final class Lockable<T> {
    ...
    public Optional<Inner> tryLock() {
        if (isLocked.compareAndSet(false, true)) {
            ...
            try {
                return timeLock.lock(request).getTokenOrEmpty().map(lockToken -> new Inner(lockToken));
            } catch (Exception e) {
    ...
    public final class Inner implements AutoCloseable {
        private final LockToken lockToken;

        private Inner(LockToken lockToken) {
