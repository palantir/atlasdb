...
public class ShardedSweepableBucketRetrieverTest {
    ...
    private final SettableRefreshable<Duration> minimumBackoff = Refreshable.create(Duration.ofMillis(0));
    private final SettableRefreshable<Duration> maxJitter = Refreshable.create(Duration.ofMillis(0));
    private final TestShardedRetrieveStrategy strategy = new TestShardedRetrieveStrategy();
    ...
    @Test
    public void returnedListIsOrderedByBucketIdentifier() {
        numShards.update(100);
        List<SweepableBucket> buckets = retriever.getSweepableBuckets();
        List<Long> bucketIds =
                buckets.stream().map(SweepableBucket::bucketIdentifier).collect(Collectors.toList());
        assertThat(bucketIds).isSorted();
    }
    ...
    public void updatesToMaxParallelismAreReflectedInSubsequentRequests() {
        ...
        retriever.getSweepableBuckets();
        maxParallelism.update(42);
        retriever.getSweepableBuckets();
        assertThat(parallelTaskExecutor.getLastMaxParallelism()).contains(42);
    }

    @Test
    public void atLeastMinimumBackoffBetweenRequests() {}

    @Test
    public void backoffCanBeInterrupted() {}

    private static final class TestShardedRetrieveStrategy implements ShardedRetrieverStrategy {
