...
public abstract class AbstractSerializableTransactionTest extends AbstractTransactionTest {
    ...
    public void testColumnRangeReadWriteConflict() {
        ...
        Map<byte[], BatchingVisitable<Map.Entry<Cell, byte[]>>> columnRange =
                t1.getRowsColumnRange(TEST_TABLE, ImmutableList.of(row), new BatchColumnRangeSelection(PtBytes.EMPTY_BYTE_ARRAY, PtBytes.EMPTY_BYTE_ARRAY, 1));
    ...
    public void testColumnRangeReadWriteEmptyRange() {
        ...
        Transaction t1 = startTransaction();
        Map<byte[], BatchingVisitable<Map.Entry<Cell, byte[]>>> columnRange =
                t1.getRowsColumnRange(TEST_TABLE, ImmutableList.of(row), new BatchColumnRangeSelection(PtBytes.toBytes("col"), PtBytes.toBytes("col0"), 1));
