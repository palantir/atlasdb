...
public final class UserPhotosStreamStore extends AbstractPersistentStreamStore {
  ...
  protected void storeBlock(Transaction t, long id, long blockNumber, final byte[] block) {
    ...
    final UserPhotosStreamValueTable.UserPhotosStreamValueRow row =
        UserPhotosStreamValueTable.UserPhotosStreamValueRow.of(id, blockNumber);
    try {
  ...
  private void touchMetadataWhileStoringForConflicts(Transaction t, Long id, long blockNumber) {
    ...
    UserPhotosStreamMetadataTable.UserPhotosStreamMetadataRow row =
        UserPhotosStreamMetadataTable.UserPhotosStreamMetadataRow.of(id);
    ...
    for (Entry<Long, StreamMetadata> e : streamIdsToMetadata.entrySet()) {
      ...
      if (metadata.getStatus() == Status.STORED) {
        ...
        rowsToStoredMetadata.put(
            UserPhotosStreamMetadataTable.UserPhotosStreamMetadataRow.of(streamId), metadata);
  ...
  protected void loadSingleBlockToOutputStream(
    ...
    UserPhotosStreamValueTable.UserPhotosStreamValueRow row =
        UserPhotosStreamValueTable.UserPhotosStreamValueRow.of(streamId, blockId);
    try {
  ...
  protected Map<Long, StreamMetadata> getMetadata(Transaction t, Set<Long> streamIds) {
    ...
    Map<UserPhotosStreamMetadataTable.UserPhotosStreamMetadataRow, StreamMetadata> metadatas =
        table.getMetadatas(getMetadataRowsForIds(streamIds));
    Map<Long, StreamMetadata> ret = Maps.newHashMap();
    for (Map.Entry<UserPhotosStreamMetadataTable.UserPhotosStreamMetadataRow, StreamMetadata> e :
        metadatas.entrySet()) {
  ...
  public Map<Sha256Hash, Long> lookupStreamIdsByHash(Transaction t, final Set<Sha256Hash> hashes) {
    ...
    Set<UserPhotosStreamHashAidxTable.UserPhotosStreamHashAidxRow> rows =
  ...
  private Set<UserPhotosStreamHashAidxTable.UserPhotosStreamHashAidxRow> getHashIndexRowsForHashes(
  ...
  private Set<UserPhotosStreamMetadataTable.UserPhotosStreamMetadataRow> getMetadataRowsForIds(
  ...
  private void putHashIndexTask(
    ...
    for (Entry<UserPhotosStreamMetadataTable.UserPhotosStreamMetadataRow, StreamMetadata> e :
      ...
      UserPhotosStreamHashAidxTable.UserPhotosStreamHashAidxRow hashRow =
          UserPhotosStreamHashAidxTable.UserPhotosStreamHashAidxRow.of(hash);
      UserPhotosStreamHashAidxTable.UserPhotosStreamHashAidxColumn column =
          UserPhotosStreamHashAidxTable.UserPhotosStreamHashAidxColumn.of(row.getId());
      UserPhotosStreamHashAidxTable.UserPhotosStreamHashAidxColumnValue columnValue =
          UserPhotosStreamHashAidxTable.UserPhotosStreamHashAidxColumnValue.of(column, 0L);
  ...
  void deleteStreams(Transaction t, final Set<Long> streamIds) {
    ...
    Set<UserPhotosStreamValueTable.UserPhotosStreamValueRow> streamValueToDelete =
        Sets.newHashSet();
    for (Entry<UserPhotosStreamMetadataTable.UserPhotosStreamMetadataRow, StreamMetadata> e :
        metadatas.entrySet()) {
      ...
      for (long i = 0; i < blocks; i++) {
        streamValueToDelete.add(
            UserPhotosStreamValueTable.UserPhotosStreamValueRow.of(streamId, i));
      }
      ...
      UserPhotosStreamHashAidxTable.UserPhotosStreamHashAidxRow hashRow =
          UserPhotosStreamHashAidxTable.UserPhotosStreamHashAidxRow.of(hash);
      UserPhotosStreamHashAidxTable.UserPhotosStreamHashAidxColumn column =
          UserPhotosStreamHashAidxTable.UserPhotosStreamHashAidxColumn.of(streamId);
  ...
  protected void markStreamsAsUsedInternal(
    ...
    for (Map.Entry<Long, byte[]> entry : streamIdsToReference.entrySet()) {
      ...
      UserPhotosStreamIdxTable.UserPhotosStreamIdxColumn col =
          UserPhotosStreamIdxTable.UserPhotosStreamIdxColumn.of(reference);
      UserPhotosStreamIdxTable.UserPhotosStreamIdxColumnValue value =
          UserPhotosStreamIdxTable.UserPhotosStreamIdxColumnValue.of(col, 0L);
  ...
  protected void touchMetadataWhileMarkingUsedForConflicts(Transaction t, Iterable<Long> ids) {
    ...
    SetView<UserPhotosStreamMetadataTable.UserPhotosStreamMetadataRow> missingRows =
