...
public interface CoordinationStore<T> {
     ...
     * @return a {@link TransformResult} indicating if the proposal was successful and the current value
     *
     * @throws IllegalStateException if the proposal fails, but no current value exists
     */
    TransformResult<ValueAndBound<T>> transformAgreedValue(Function<ValueAndBound<T>, T> transform);
}
