...
public final class AtlasDbDialogueFactories {
    ...
    public static DialogueClients.ReloadingFactory decorate(
            DialogueClients.ReloadingFactory baseFactory,
            Refreshable<ServerListConfig> serverListConfig,
            AuxiliaryRemotingParameters parameters,
            String serviceName) {
        return baseFactory.reloading(serverListConfig.map(
                        config -> DialogueClientOptions.toServicesConfigBlock(config, parameters, serviceName)))
                .withFailedUrlCooldown(
                        parameters.shouldRetry() ? STANDARD_FAILED_URL_COOLDOWN : NON_RETRY_FAILED_URL_COOLDOWN)
