...
public class BenchmarksResource implements BenchmarksService {
    ...
    public BenchmarksResource(AtlasDbConfig config, SerializableTransactionManager txnManager) {
        this.config = config;
    ...
    public Map<String, Object> readTransactionRows(int numClients, int numRequestsPerClient, int numRows,
            int dataSize, int numUpdatesPerCell) {
    ...
    @Override
    public Map<String, Object> rangeScanRows(int numClients, int numRequestsPerClient, int numRows, int dataSize, int numUpdates, int numDeleted) {
        return RowsRangeScanBenchmark.execute(txnManager, numClients, numRequestsPerClient, numRows, dataSize, numUpdates, numDeleted);
    }

    @Override
    public Map<String, Object> rangeScanDynamicColumns(int numClients, int numRequestsPerClient, int numRows,
            int dataSize, int numUpdates, int numDeleted) {
        return DynamicColumnsRangeScanBenchmark.execute(txnManager, numClients, numRequestsPerClient, numRows,
                dataSize, numUpdates, numDeleted);
    }
}
