...
import com.palantir.atlasdb.ete.utilities.CassandraCommandsV2;
...
public abstract class AbstractCassandraTimestampTest {
    ...
    public void timestampsForRangeTombstonesAreCurrentInThoroughTables() throws IOException, InterruptedException {
        ...
        CassandraCommandsV2.nodetoolFlush(CASSANDRA_CONTAINER_NAME);
        CassandraCommandsV2.nodetoolCompact(CASSANDRA_CONTAINER_NAME);

        List<String> ssTables = CassandraCommandsV2.nodetoolGetSsTables(
                CASSANDRA_CONTAINER_NAME, "atlasete", TodoSchema.todoTable(), ValueType.FIXED_LONG.convertFromJava(ID));
        String sstableMetadata =
                CassandraCommandsV2.ssTableMetadata(CASSANDRA_CONTAINER_NAME, Iterables.getOnlyElement(ssTables));
