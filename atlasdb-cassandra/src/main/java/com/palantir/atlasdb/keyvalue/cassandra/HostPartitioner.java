...
final class HostPartitioner {
    ...
    static <V> Map<InetSocketAddress, List<V>> partitionByHost(CassandraClientPool clientPool,
        ...
        return partitionByHost(clientPool, Streams.stream(iterable), keyExtractor);
    }
    ...
    static <V> Map<InetSocketAddress, List<V>> partitionByHost(CassandraClientPool clientPool,
        ...
        ListMultimap<ByteBuffer, V> partitionedByKey =
                stream.collect(ImmutableListMultimap.toImmutableListMultimap(
                        keyExtractor.andThen(ByteBuffer::wrap),
                        x -> x));
