...
public class CassandraTimestampStore {
     ...
     */
    public synchronized TimestampBoundStoreEntry getUpperLimitEntry() {
        return cassandraKeyValueService.clientPool.runWithRetry(client -> {
            BoundData boundData = getCurrentBoundData(client);
            return TimestampBoundStoreEntry.createFromBytes(boundData.bound());
        });
    }
     ...
     */
    public synchronized CqlResult storeTimestampBound(TimestampBoundStoreEntry expected, long target) {
        return cassandraKeyValueService.clientPool.runWithRetry(client -> {
            byte[] expectedBytes = expected.getByteValue();
            byte[] targetBytes = TimestampBoundStoreEntry.getByteValueForIdAndBound(id, target);
            ...
            return executeQueryUnchecked(client, queryBuffer);
        });
    }
    ...
    private static ConcurrentModificationException constructConcurrentTimestampStoreException(
        ...
        ConcurrentModificationException except = new ConcurrentModificationException(
                String.format(replaceBracesWithStringFormatSpecifier(msg),
                        ...
                        actual,
                        currentId));
        log.error(msg, expectedValueString, target, expectedValueString, expectedIdString, actual, currentId);
