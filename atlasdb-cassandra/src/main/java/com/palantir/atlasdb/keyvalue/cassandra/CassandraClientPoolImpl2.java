...
//public class CassandraClientPoolImpl2 implements CassandraClientPool {
...
//
//    @VisibleForTesting
//    static CassandraClientPoolImpl2 createImplForTest(
//            MetricsManager metricsManager,
//            CassandraKeyValueServiceConfig config,
//            StartupChecks startupChecks,
//            Blacklist blacklist) {
//        CassandraRequestExceptionHandler exceptionHandler = testExceptionHandler(blacklist);
//        CassandraClientPoolImpl2 cassandraClientPool = new CassandraClientPoolImpl2(
//                metricsManager,
//                config,
//                startupChecks,
//                exceptionHandler,
...
//
//    @Override
//    public <V, K extends Exception> ListenableFuture<V> runWithRetryOnHostAsync(InetSocketAddress specifiedHost,
//            AsyncFunctionCheckedException<AsyncCassandraClient, V, K> fn) throws K {
//
//        // TODO(jakubk): Do retries!!!!!
//        RetryableCassandaRequestContext ctx = new RetryableCassandaRequestContext(specifiedHost);
//        return runSingle(ctx, fn);
//    }
//
//    private <V, K extends Exception> ListenableFuture<V> runSingle(RetryableCassandaRequestContext req,
//            AsyncFunctionCheckedException<AsyncCassandraClient, V, K> fn) throws K {
//        if (log.isTraceEnabled()) {
//            log.trace("Running function on host {}.",
//                    SafeArg.of("host", CassandraLogHelper.host(req.getPreferredHost())));
//        }
//        CassandraService.CassandraClients hostPool = getPreferredClientsOrFallBack(req);
//        return Futures.transform(runWithPooledResourceRecordingMetricsAsync(hostPool, fn),
//                result -> {
//                    removeFromBlacklistAfterResponse(hostPool.poolingContainer().getHost());
//                    return result;
//                }, MoreExecutors.directExecutor());
//    }
//
//    private <V, K extends Exception> CassandraService.CassandraClients getPreferredClientsOrFallBack(
//            RetryableCassandaRequestContext req) {
//        CassandraService.CassandraClients hostPool = cassandra.getAllPools().get(req.getPreferredHost());
//
//        if (blacklist.contains(req.getPreferredHost()) || hostPool == null || req.shouldGiveUpOnPreferredHost()) {
//            InetSocketAddress previousHost =
//                    hostPool == null ? req.getPreferredHost() : hostPool.poolingContainer().getHost();
//            Optional<CassandraService.CassandraClients> hostPoolCandidate
//                    = cassandra.getRandomGoodCassandraClientsForPredicate(address -> !req.alreadyTriedOnHost(address));
//            hostPool = hostPoolCandidate.orElseGet(cassandra::getRandomGoodClients);
//            log.warn("Randomly redirected a query intended for host {} to {}.",
//                    SafeArg.of("previousHost", CassandraLogHelper.host(previousHost)),
//                    SafeArg.of("randomHost", CassandraLogHelper.host(hostPool.poolingContainer().getHost())));
//        }
//        return hostPool;
//    }
//
...
//    }
//
//    private <V, K extends Exception> ListenableFuture<V> runWithPooledResourceRecordingMetricsAsync(
//            CassandraService.CassandraClients hostPool,
//            AsyncFunctionCheckedException<AsyncCassandraClient, V, K> fn) throws K {
//        metrics.recordRequestOnHost(hostPool.poolingContainer());
//        return Futures.catching(
//                fn.apply(hostPool.asyncClient()),
//                Exception.class,
//                e -> {
//                    metrics.recordExceptionOnHost(hostPool.poolingContainer());
//                    if (CassandraRequestExceptionHandler.isConnectionException(e)) {
//                        metrics.recordConnectionExceptionOnHost(hostPool.poolingContainer());
//                    }
