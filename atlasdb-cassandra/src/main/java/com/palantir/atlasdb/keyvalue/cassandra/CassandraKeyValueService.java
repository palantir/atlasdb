...
public class CassandraKeyValueService extends AbstractKeyValueService {
    ...
    private void upgradeFromOlderInternalSchema() {
        try {
            ...
            for (CfDef clusterSideCf : knownCfs) {
                ...
                } else if (!tableName.equals(CassandraConstants.METADATA_TABLE)) { // only expected caseƒ
    ...
    private Set<String> filterOutTrulyEmptyTables(Set<String> tableNames) {
        ...
        for (final String tableName : tableNames) {
            ...
            try {
                ...
                log.warn("Table " + tableName + " could not be checked for emptiness. Proceeding with requested operation without optimization.");
            }
    ...
    public Set<String> getAllTableNames() {
        final CassandraKeyValueServiceConfig config = configManager.getConfig();
        try {
            return clientPool.runWithRetry(new FunctionCheckedException<Client, Set<String>, Exception>() {
                @Override
