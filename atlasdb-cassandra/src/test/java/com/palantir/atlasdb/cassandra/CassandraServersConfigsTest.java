...
public class CassandraServersConfigsTest {
    ...
    private static final InetSocketAddress THRIFT_SERVER_1 = InetSocketAddress.createUnresolved("foo", TEST_THRIFT_PORT);
    private static final InetSocketAddress THRIFT_SERVER_2 = InetSocketAddress.createUnresolved("bar", TEST_THRIFT_PORT);
    ...
    private static List<InetSocketAddress> constructHosts(int port, String[] hosts) {
        return Stream.of(hosts).map(host -> InetSocketAddress.createUnresolved(host, port)).collect(Collectors.toList());
    }
