...
public class CassandraClientPoolTest {
    ...
    public void shouldReturnAddressForSingleServer() throws UnknownHostException {
        CassandraClientPoolImpl cassandraClientPool = clientPoolWithServers(ImmutableSet.of(HOST_1));
    ...
    public void shouldThrowIfPortsAreNotTheSameAddressDoesNotMatch() throws UnknownHostException {
        ...
        CassandraClientPoolImpl cassandraClientPool = clientPoolWithServers(ImmutableSet.of(HOST_1, host2));
    ...
    private CassandraClientPoolImpl clientPoolWith(
        ...
        CassandraClientPoolImpl cassandraClientPool = CassandraClientPoolImpl.createImplForTest(config,
    ...
    private void runNoopOnHost(InetSocketAddress host, CassandraClientPoolImpl pool) {
        pool.runOnHost(host, input -> null);
    }

    private void runNoopWithRetryOnHost(InetSocketAddress host, CassandraClientPoolImpl pool) {
        pool.runWithRetryOnHost(host, input -> null);
    }

    private void runNoopOnHostWithException(InetSocketAddress host, CassandraClientPoolImpl pool) {
        try {
            pool.runOnHost(host, input -> null);
            fail();
        } catch (Exception e) {
    ...
    private void runNoopOnHostWithRetryWithException(InetSocketAddress host, CassandraClientPoolImpl pool) {
        try {
            pool.runWithRetryOnHost(host, input -> null);
            fail();
        } catch (Exception e) {
