...
@Command(
        ...
        description = "Clean out the entries in a _transactions table for the "
...
public class CleanTransactionRange extends AbstractTimestampCommand {
    ...
    private static final OutputPrinter printer =
            new OutputPrinter(LoggerFactory.getLogger(CleanTransactionRange.class));

    @Option(
            name = {"-s", "--start-timestamp"},
            title = "TIMESTAMP",
    ...
    @Option(
            name = {"--skip-start-timestamp-check"},
            title = "TIMESTAMP",
            type = OptionType.GROUP,
    ...
    protected int executeTimestampCommand(AtlasDbServices services) {
        new V1TransactionsTableRangeDeleter(
                        ...
                        printer)
                .deleteRange(TimestampRange.createInclusiveRange(timestamp, Long.MAX_VALUE));
        return 0;
    }
}
