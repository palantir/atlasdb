...
public class SweepBenchmarks {
    ...
    @Measurement(time = 300, timeUnit = TimeUnit.SECONDS)
    public Object longSweepRun(RegeneratingTable.SweepHundredColumnsDeterministicRegeneratingTable table) {
        SweepTaskRunner sweepTaskRunner = table.getSweepTaskRunner();
        SweepResults sweepResults = sweepTaskRunner.run(table.getTableRef(), BATCH_SIZE, BATCH_SIZE, null);
        assertThat(sweepResults.getCellsDeleted(), is(9900L));
        return sweepResults;
    }
}
