...
public class TempBenchmark {
    private static final int BATCH_SIZE_HINT = 1000;

    @Benchmark
    @Threads(1)
    @Warmup(time = 1)
    @Measurement(time = 1)
    public Object fullTableScan_Clean_Huge_Parallel(ConsecutiveNarrowTable.CleanNarrowTable table) {
        return fullTableScan(table, BATCH_SIZE_HINT);
    }
    ...
    @Warmup(time = 1)
    @Measurement(time = 1)
    public Object fullTableScan_Dirty_Huge_Parallel(ConsecutiveNarrowTable.DirtyNarrowTable table) {
        return fullTableScan(table, BATCH_SIZE_HINT);
    }


    private int fullTableScan(ConsecutiveNarrowTable table, int batchSizeHint) {
        int numCellsExpected = table.getNumRows() * (table instanceof ConsecutiveNarrowTable.DirtyNarrowTable ? 100 : 1);
