...
public interface NamespacedConjureTimelockService {
    ConjureUnlockResponseV2 unlock(ConjureUnlockRequestV2 request);

    ConjureRefreshLocksResponseV2 refreshLocks(ConjureRefreshLocksRequestV2 request);

    ConjureWaitForLocksResponse waitForLocks(ConjureLockRequest request);

    ConjureLockResponseV2 lock(ConjureLockRequest request);
