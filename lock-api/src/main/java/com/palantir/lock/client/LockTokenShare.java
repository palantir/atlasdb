...
final class LockTokenShare implements LockToken {
     ...
     */
    static List<LockToken> share(LockToken token, int referenceCount) {
        com.palantir.logsafe.Preconditions.checkArgument(referenceCount > 0, "Reference count should be more than zero");
        com.palantir.logsafe.Preconditions.checkArgument(!(token instanceof LockTokenShare), "Can not share a shared lock token");
    ...
    private static final class ReferenceCounter {
        ...
        synchronized boolean unmark() {
            com.palantir.logsafe.Preconditions.checkState(referenceCount > 0, "Reference count can not go below zero!");
            referenceCount--;
