    ...
    static Consumer<List<BatchElement<NamespacedRequest, GetCommitTimestampResponse>>> consumer(
        ...
        return batch -> {
            ...
            while (batchStateManager.hasPendingRequests()) {
                batchStateManager.processResponse(
                        KeyedStream.stream(delegate.getCommitTimestamps(batchStateManager.getRequests()))
                                .map(LockLeaseService::assignLeasedLockTokenToImmutableCommitTimestampLock)
                                .collectToMap());
            }
        };
    }
