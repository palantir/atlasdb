...
@Path("/lock")
@Beta public interface AuthedLockService extends AuthedRemoteLockService {
    /**
    ...
    @NonIdempotent
    LockResponse lockWithFullLockResponse(@HeaderParam(HttpHeaders.AUTHORIZATION) AuthHeader authHeader,
            @Safe @PathParam("client") LockClient client, LockRequest request)
            throws InterruptedException;

    /**
     * Unlocks a lock, given a provided lock token.
     *
     * @deprecated use {@link #unlockSimple(AuthHeader, SimpleHeldLocksToken)} instead
     * @see #unlockSimple(AuthHeader, SimpleHeldLocksToken)
     */
    @POST
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Deprecated
    @NonIdempotent boolean unlock(@HeaderParam(HttpHeaders.AUTHORIZATION) AuthHeader authHeader, HeldLocksToken token);

    @POST
    @Path("unlock-simple")
    @Produces(MediaType.APPLICATION_JSON)
    @Consumes(MediaType.APPLICATION_JSON)
    @NonIdempotent boolean unlockSimple(@HeaderParam(HttpHeaders.AUTHORIZATION) AuthHeader authHeader,
            SimpleHeldLocksToken token);
     ...
     * equivalent to {@link #unlock(AuthHeader, HeldLocksToken)} except for error
    ...
    @Produces(MediaType.APPLICATION_JSON)
    @Consumes(MediaType.APPLICATION_JSON)
    @NonIdempotent boolean unlockAndFreeze(@HeaderParam(HttpHeaders.AUTHORIZATION) AuthHeader authHeader,
    ...
    @Produces(MediaType.APPLICATION_JSON)
    @Consumes(MediaType.APPLICATION_JSON)
    @Idempotent Set<HeldLocksToken> getTokens(@HeaderParam(HttpHeaders.AUTHORIZATION) AuthHeader authHeader,
            @Safe @PathParam("client") LockClient client);

    /**
     * Refreshes the given lock tokens.
     *
     * @deprecated use {@link #refreshLockRefreshTokens(AuthHeader, Iterable)} instead
     * @see #refreshLockRefreshTokens(AuthHeader, Iterable)
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Deprecated
    @Idempotent Set<HeldLocksToken> refreshTokens(@HeaderParam(HttpHeaders.AUTHORIZATION) AuthHeader authHeader,
            Iterable<HeldLocksToken> tokens);
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Idempotent
    @Nullable HeldLocksGrant refreshGrant(@HeaderParam(HttpHeaders.AUTHORIZATION) AuthHeader authHeader,
    ...
    @Produces(MediaType.APPLICATION_JSON)
    @Consumes(MediaType.APPLICATION_JSON)
    @NonIdempotent HeldLocksGrant convertToGrant(@HeaderParam(HttpHeaders.AUTHORIZATION) AuthHeader authHeader,
            HeldLocksToken token);
    ...
    @Produces(MediaType.APPLICATION_JSON)
    @Consumes(MediaType.APPLICATION_JSON)
    @NonIdempotent HeldLocksToken useGrant(@HeaderParam(HttpHeaders.AUTHORIZATION) AuthHeader authHeader,
            @Safe @PathParam("client") LockClient client, HeldLocksGrant grant);
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @NonIdempotent HeldLocksToken useGrant(@HeaderParam(HttpHeaders.AUTHORIZATION) AuthHeader authHeader,
            @Safe @PathParam("client") LockClient client, BigInteger grantId);

    /**
     * This method is the same as <code>getMinLockedInVersionId(LockClient.ANONYMOUS)</code>.
     * @deprecated use {@link #getMinLockedInVersionId(AuthHeader, LockClient)} instead
     */
    @POST
    ...
    @Produces(MediaType.APPLICATION_JSON)
    @Consumes(MediaType.APPLICATION_JSON)
    Long getMinLockedInVersionId(@HeaderParam(HttpHeaders.AUTHORIZATION) AuthHeader authHeader,
            @Safe @PathParam("client") LockClient client);
    ...
    @Produces(MediaType.APPLICATION_JSON)
    @Consumes(MediaType.APPLICATION_JSON)
    @Idempotent LockServerOptions getLockServerOptions(@HeaderParam(HttpHeaders.AUTHORIZATION) AuthHeader authHeader);
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Override
    @Idempotent void logCurrentState(@HeaderParam(HttpHeaders.AUTHORIZATION) AuthHeader authHeader);
