...
public class LockRefresher {
    ...
    private final Set<LockRefreshToken> tokensToRefresh = Sets.newConcurrentHashSet();
    ...
    private void refreshLocks() {
        try {
            Set<LockRefreshToken> toRefresh = ImmutableSet.copyOf(tokensToRefresh);
            if (toRefresh.isEmpty()) {
                return;
            }

            Set<LockRefreshToken> refreshed = timelockService.refreshLockLeases(toRefresh);
    ...
    public void registerLock(LockRefreshToken token) {
        tokensToRefresh.add(token);
    }

    public void unregisterLocks(Collection<LockRefreshToken> tokens) {
