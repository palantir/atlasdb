...
public final class NewLocksVisitor implements LockWatchEvent.Visitor<Set<LockDescriptor>> {
    private final LockToken lockToIgnore;

    private NewLocksVisitor(LockToken lockToIgnore) {
        this.lockToIgnore = lockToIgnore;
    }

    public static NewLocksVisitor ignoring(LockToken token) {
        return new NewLocksVisitor(token);
    }

    @Override
    public Set<LockDescriptor> visit(LockEvent lockEvent) {
        return lockEvent.lockToken().equals(lockToIgnore) ? ImmutableSet.of() : lockEvent.lockDescriptors();
    }
