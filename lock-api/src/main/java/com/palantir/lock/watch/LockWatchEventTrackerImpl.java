...
public class LockWatchEventTrackerImpl implements LockWatchEventTracker {
    ...
    public VersionedLockWatchState currentState() {
        lock.readLock().lock();
        try {
            return VersionedLockWatchStateImpl
                    .withoutLastUpdate(lastKnownVersion, watches.get(), singleLocks.get(), leaderId);
        } finally {
    ...
    @Override
    public synchronized void updateState(LockWatchStateUpdate update) {
        if (!update.success()) {
