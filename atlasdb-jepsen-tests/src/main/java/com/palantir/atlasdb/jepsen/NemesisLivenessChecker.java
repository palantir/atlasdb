...
/**
 * This checker verifies that during the interval from a nemesis's start action to its stop action, it was
 * always possible for a request to be serviced. That is, some Jepsen worker must be associated with BOTH an invoke
 * and an ok event during the relevant interval.
 *
 * This checker should only be used if it is expected that the cluster is able to successfully recover from all
 * possible invocations of the nemesis within the time interval where the nemesis is down.
 */
public class NemesisLivenessChecker implements Checker {
    @Override
