...
public final class TimeLockCorruptionTestSetup {
    ...
    void setup() {
        try {
            File randomFile1 = Files.createTempDirectory("randomFile1").toFile();
            randomFile1.deleteOnExit();
            File randomFile2 = Files.createTempDirectory("randomFile2").toFile();
            randomFile1.deleteOnExit();
            File randomFile3 = Files.createTempDirectory("randomFile3").toFile();
            randomFile1.deleteOnExit();

            localDataSource = SqliteConnections.getDefaultConfiguredPooledDataSource(randomFile1.toPath());
            remoteDataSource1 = SqliteConnections.getDefaultConfiguredPooledDataSource(randomFile2.toPath());
            remoteDataSource2 = SqliteConnections.getDefaultConfiguredPooledDataSource(randomFile3.toPath());
